...
import { Loader2, Check, X as XIcon, BookOpen, CalendarDays, AwardIcon, ChevronDown } from "lucide-react"; // Ditambahkan ikon
...
export default function EditEducationForm({ initialData, onSubmitSuccess, onCancel }: EditEducationFormProps) {
  const [level, setLevel] = useState(initialData.level || "");
  const [customLevel, setCustomLevel] = useState(levels.includes(initialData.level) ? "" : initialData.level);
  ...
  const [loading, setLoading] = useState(false); // Renamed from isSubmitting for consistency with other forms
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const user = (await import("../../firebase")).auth.currentUser; // Dinamis import firebase
      if (!user) {
      ...
      const res = await fetch(
        `https://jobseeker-capstone-705829099986.asia-southeast2.run.app/education/${initialData.id}`,
        {
          method: "PATCH",
          headers: {
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.error || "Gagal mengupdate pendidikan");
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      {/* Judul form sudah ada di modal container, jadi tidak perlu di sini lagi */}
      {/* <h3 className="text-xl font-semibold text-gray-800 mb-6">Edit Pendidikan</h3> */}
