...
export default function EditExperienceForm({ initialData, onSubmitSuccess, onCancel }: EditExperienceFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(
        `https://jobseeker-capstone-705829099986.asia-southeast2.run.app/experience/${initialData.id}`,
        {
          method: "PATCH",
          headers: {
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.error || "Gagal mengupdate pengalaman");
      }
      Swal.fire({
        title: "Sukses!",
        text: "Pengalaman berhasil diupdate.",
        icon: "success",
      ...
      if (err instanceof Error) {
        ...
        Swal.fire({ title: "Gagal!", text: "Terjadi kesalahan saat mengupdate data.", icon: "error", confirmButtonColor: "#3085d6" });
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      {/* <h3 className="text-xl font-semibold text-gray-800 mb-6">Edit Pengalaman Kerja</h3> */}
      ...
      {formField("Deskripsi Pekerjaan (Opsional)", "description", description, (e) => setDescription(e.target.value), "Jelaskan tanggung jawab dan pencapaian Anda...", "text", false, undefined, <AlignLeft />, true)}
