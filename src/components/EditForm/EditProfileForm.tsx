...
import { UserCircle, UploadCloud, Loader2, Check, X as XIcon } from "lucide-react"; // Menambahkan ikon Loader2, Check, XIcon
...
interface EditProfileFormProps {
  initialData: Omit<ProfileData, "photoFile" | "photoPreviewUrl"> & { photoUrl?: string };
...
export default function EditProfileForm({ initialData, onSubmit, onCancel }: EditProfileFormProps) {
  ...
  // Helper function untuk rendering input field yang konsisten
  const formField = (
    label: string,
    name: keyof Omit<ProfileData, 'photoFile' | 'photoPreviewUrl'>, // Pastikan name sesuai dengan key di ProfileData
    placeholder: string,
    type: string = "text",
    ...
    <div>
      ...
      <div className="relative">
        ...
        <input
          ...
          id={name}
          name={name}
          value={formData[name] as string} // formData[name] akan string untuk field ini
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      {/* Judul form biasanya ada di kontainer modal, bisa di-remark jika form ini adalah konten modal */}
      {/* <h3 className="text-xl font-semibold text-gray-800 mb-6">Edit Detail Profil</h3> */}
      ...
      <div className="flex flex-col items-center space-y-4">
        {formData.photoPreviewUrl ? (
          ...
          <div className="w-32 h-32 rounded-full bg-gray-200 flex items-center justify-center text-gray-400 border-4 border-gray-100 shadow-md">
            <UserCircle size={72} /> {/* Ukuran ikon placeholder disesuaikan */}
          </div>
        )}
        <label
          htmlFor="photoFile"
